    ...
    if os.path.exists(filename):
        with open(filename, 'rb') as f:
            ...
            if crl.next_update_utc > datetime.datetime.now(UTC):
                print(f"CRL en cache utilisée : {crl_url}")
                return crl
            else:
                print(f"CRL expirée, nouveau téléchargement : {crl_url}")

    print(f"Téléchargement CRL : {crl_url}")
                ...
                if uri.value.startswith("http"):
                    ...
                    for revoked_cert in crl:
                        if revoked_cert.serial_number == cert.serial_number:
                            print("Certificat révoqué selon la CRL.")
                            return False
        print("Certificat non révoqué selon la CRL.")
        return True
    except Exception as e:
        print(f"Erreur vérification CRL : {e}")
        return True
...
def check_revocation_ocsp(cert, issuer_cert):
    try:
        ...
        if not ocsp_urls:
            print("Aucun point OCSP trouvé.")
            return True

        ocsp_url = ocsp_urls[0]
        print(f"Vérification OCSP via {ocsp_url}")
        ...
        if ocsp_response.response_status.name != "SUCCESSFUL":
            print(f"OCSP échoué : {ocsp_response.response_status}")
            return False

        status = ocsp_response.certificate_status
        if status == cryptography.x509.ocsp.OCSPCertStatus.GOOD:
            print("Certificat valide selon OCSP.")
            return True
        elif status == cryptography.x509.ocsp.OCSPCertStatus.REVOKED:
            print("Certificat révoqué selon OCSP.")
            return False
        elif status == cryptography.x509.ocsp.OCSPCertStatus.UNKNOWN:
            print("Certificat inconnu dans l’OCSP (statut 'unknown'). On continue par précaution.")
            return True
        else:
            print(f"Statut OCSP inattendu : {status}")
            return False
    except Exception as e:
...
# === MAIN ===
def main():
    parser = argparse.ArgumentParser(description="Validation d'une chaîne de certificats X.509 avec CRL et OCSP")
    ...
    for i in range(1, len(certificates)):
        ...
        print(f"\n Vérification révocation pour le certificat {i}...")
        if not check_revocation_crl(cert, issuer):
            print("Révocation CRL détectée.")
            sys.exit(1)
        if not check_revocation_ocsp(cert, issuer):
            print("Révocation OCSP détectée.")
            sys.exit(1)

    print("\n Tous les certificats sont valides et non révoqués !")
