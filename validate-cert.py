...
def get_hash_function(hash_algorithm):
    """Convert cryptography hash algorithm to hashlib equivalent"""
...
def verify_signature_rsa(public_key, signature, data, hash_algorithm):
    """ Vérification RSA en utilisant des opérations sur grands nombres. """
    ...
    # Convert the decrypted signature to bytes with proper padding length
...
def verify_signature_ecdsa(public_key, signature, data, hash_algorithm):
    """ Vérification ECDSA en utilisant la bibliothèque ecdsa. """
    try:
...
def verify_certificate_chain(certificates):
    """Vérifie la chaîne de certificats dans l'ordre reçu (Root -> Intermédiaire -> Leaf)."""
    for i in range(len(certificates) - 1):
        parent_cert = certificates[i]     # Le parent doit être un CA
        child_cert = certificates[i + 1]  # Le certificat signé par le parent
...
def check_basic_constraints(cert):
    try:
        ...
        print(f"⚠️ Avertissement: Extension BasicConstraints non trouvée")
        return False
...
def check_key_usage(cert, is_ca):
    try:
        ...
        if not is_ca and key_usage.key_cert_sign:
            print("⚠️ Avertissement: Certificat non-CA avec l'autorisation key_cert_sign")
        return True
    except cryptography.x509.ExtensionNotFound:
        print(f"⚠️ Avertissement: Extension KeyUsage non trouvée")
        return False
